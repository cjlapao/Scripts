...
class SendEmail
	def initialize(_error,_errorcode)
		begin
			if _error
				_subject = "[Failure] BCKMTA syncronization failed"
				_body = "Alara Systems\n\nThe Backup MTA syncronization failed with the following output\n"
...
class MtaSync
	def initialize
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /var/mail/virtual/ root@mailbck:/var/mail/virtual") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
