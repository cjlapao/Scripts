...
require 'open3'
$logdir 					= '/var/log/settings-mail-sync.log'
$_smtp						= '192.168.1.2'
...
class MtaSync
	...
	def syncSpamassassin
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/spamassassin/ root@mailbck:/etc/spamassassin") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
	...
	def syncPostfix
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/postfix/ root@mailbck:/etc/postfix") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncCourier
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/courier/ root@mailbck:/etc/courier") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncAmavis
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/amavis/ root@mailbck:/etc/amavis") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncPostgrey
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/postgrey/ root@mailbck:/etc/postgey") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncOpendkim
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/opendkim/ root@mailbck:/etc/opendkim") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncOpendkimConf
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/opendkim.conf root@mailbck:/etc/") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncOpemdmarc
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/opendmarc/ root@mailbck:/etc/opendmarc") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncOpemdmarcConf
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/opendmarc.conf root@mailbck:/etc/") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncPostfixadmin
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /etc/postfixadmin/ root@mailbck:/etc/postfixadmin") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def syncPostfixadmindb
		...
		captured_stdout = '\n'
		captured_stderr = '\n'
		exit_status = Open3.popen3(ENV,"rsync -avz --delete /var/lib/mysql/postfixadmin/ root@mailbck:/var/lib/mysql/postfixadmin") {|stdin, stdout, stderr, wait_thr|
			@pid = wait_thr.pid
			GenerateLog.new(@pid,"Starting #{syncName} settings syncronization")
			stdin.close
			@captured_stdout = stdout.read
	...
	def restartService
		IO.popen('ssh root@mailbck "reboot"')
	end
end
sync = MtaSync.new
