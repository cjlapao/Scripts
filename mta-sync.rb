...
class MtaSync
	def initialize
		IO.popen("rsync -avz --delete /var/mail/virtual/ root@backupnas:/Backups/mta-rsync"){|_io|
			_out = _io.readlines
			output = File.open("/root/scripts/mta-sync.output","a")
			_out.each{|line|
				output.write(line)
			}
...
log = File.open("/var/log/mta-sync.log","a")
